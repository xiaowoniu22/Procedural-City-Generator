...
function generateScene() {
	...
	// .load( [
	// 	"TestRight.png", "TestLeft.png", "TestTop.png", "TestBottom.png", "TestFront.png", "TestBack.png"
	// ] );
...
function generateLighting() {
	...
	spotLight.position.set(50, 50, 60);
	...
	gltfLoader.load(
		...
		(gltf) =>
		{
			for (var i = 0; i < numberOfTrees; i++) {
				...
				var treeHeight = getRandomIntBetween(minTreeHeight, maxTreeHeight);
				gltf.scene.position.set(tree_x_coord, treeHeight/2 + curbHeight, tree_z_coord)
				trees.push(gltf.scene);
				// scene.add(gltf.scene)
			}

			if (trees.length) scene.add(trees);
		}
	)
...
function getBuildingMesh(boxGeometryParameters, position, color) {
	...
	const textureLoader = new THREE.TextureLoader();
	const texture = textureLoader.load('./textures/glass.jpg', () => {
		console.log('loading finished')
	},
		() => {
			console.log('loading progressing')
